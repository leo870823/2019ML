...
def train(device,model,optimizer,MAX_EPOCH,train_loader,val_loader):
    ...
    for epoch in range(MAX_EPOCH):
        ...
        for batch_idx,(data,label) in enumerate(train_loader):
            ...
            #print((label==pred_label).sum().item())
...
if __name__ == "__main__":
    ...
    '''
    test_dataset=test_hw3 ( sys.argv[3], range(0,7000)) 
    ...
    model=Classifier()
    model=model.to(device)
    model.load_state_dict(torch.load('./model/model_draw.pkl'))
    model.eval()
    ...
    with open('ans_draw.csv', 'w', newline='') as csvfile:
